...
def _transform(X, y, start=0, stop=-1):
    ...
    v['part_pid'] = PID
    v['part_isNeutralHadron'] = ak.values_astype(PID == 3, 'float32')
    v['part_isPhoton'] = ak.values_astype(PID == 4, 'float32')
    v['part_isChargedHadron'] = ak.values_astype(PID == 0, 'float32')
    v['part_isElectron'] = ak.values_astype(PID == 1, 'float32')
    v['part_isMuon'] = ak.values_astype(PID == 2, 'float32')
    
    v['part_charge'] = v['part_isChargedHadron'] - (v['part_isElectron']+v['part_isMuon'])
