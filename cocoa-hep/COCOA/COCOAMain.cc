...
int main(int argc, char **argv)
{
	...
	{ // batch mode
		...
		while ( std::getline(filestream, line) )
		{
			...
			else if ( (input_file_path != "") && (line.find("/generator/hepmc/open") != std::string::npos) )
			{
				if (input_file_path.find(".hepmc") != std::string::npos)
				{
					UImanager->ApplyCommand(G4String("/generator/hepmc/open " + input_file_path));
				}
				else
				{
					G4cout << "Input file is not a HepMC file (.hepmc)!" << G4endl;
					return 1;
				}
			}
			else if (line.at(0)!='#')
